-- Simplified PostgreSQL schema for memory management without pgvector

-- Drop existing tables if they exist (for clean setup)
DROP TABLE IF EXISTS memory_duplicates CASCADE;
DROP TABLE IF EXISTS memory_analytics CASCADE;
DROP TABLE IF EXISTS prompt_fragments CASCADE;
DROP TABLE IF EXISTS memories CASCADE;

-- Memories table for user-specific memories
...
CREATE TABLE IF NOT EXISTS memory_duplicates (
    id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
    original_id TEXT REFERENCES memories(id) ON DELETE CASCADE,
