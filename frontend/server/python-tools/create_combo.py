...
def create_combo_listing(client, product1_id, product2_id, sku_suffix=None, discount_amount=None, 
    ...
    # Use provided serial or default to YYMM
    if serial is None:
        serial = datetime.now().strftime('%y%m')
    ...
    if result and 'data' in result and result['data'].get('productUpdate'):
        ...
        variant_input = {
            ...
            "inventoryPolicy": "DENY"  # Don't allow oversell for combos by default
        }
        ...
        with open(csv_file, 'r') as infile, open(output_file, 'w', newline='') as outfile:
            ...
            for row in reader:
                ...
                try:
                    ...
                    prefix = row.get('prefix', '').strip() or "COMBO"
                    ...
                    result = create_combo_listing(
                        ...
                        publish, csv_writer, prefix, serial
                    )
...
def create_sample_csv():
    ...
    with open(sample_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['product1', 'product2', 'sku_suffix', 'discount_amount', 'discount_percent', 'publish', 'prefix', 'serial'])
        writer.writerow(['breville-barista-express', 'eureka-mignon-specialita', 'BE-ES1', '200', '', 'false', 'COMBO', ''])
        writer.writerow(['BES870XL', 'GRIND-01', '', '', '15', 'false', 'COMBO', ''])
        writer.writerow(['7234567890123', '9876543210987', 'CUSTOM', '150', '', 'true', 'CD25', '001'])
    ...
    print("  prefix: Optional SKU prefix (defaults to 'COMBO')")
    print("  serial: Optional serial/tracking number (defaults to YYMM)")
...
def main():
    ...
    parser.add_argument('--prefix', default='COMBO', help='SKU prefix (default: COMBO)')
    parser.add_argument('--serial', help='Serial/tracking number (default: YYMM)')
