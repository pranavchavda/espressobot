...
router.post('/run', async (req, res) => {
  ...
  try {
    ...
    const result = await run(unifiedAgent, agentInput, {
      ...
      onStepFinish: (step) => {
        ...
        if (step.type === 'tool_call' && step.tool_name === 'create_task' && step.result) {
          ...
          // Task management is now handled via agent's own tools
        }
      },
      onMessage: (message) => {
              ...
              if (toolName === 'create_task') {
                try {
                  ...
                  if (taskId) {
                    ...
                    // Task fetching is now handled via the agent's own tools
                  }
                } catch (e) {
    ...
    // Task fetching is now handled through the agent's own tools
    console.log('Task management handled by agent tools');
    
    // Send final task summary based on agent's task management
