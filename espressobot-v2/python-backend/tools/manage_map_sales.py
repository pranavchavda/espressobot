...
class MAPSalesManager:
    ...
    def update_variant_price(self, variant_id, price, compare_at_price=None):
        ...
        if compare_at_price:
            variables["input"]["compareAtPrice"] = str(compare_at_price)
        ...
        if response.get('data', {}).get('productVariantUpdate', {}).get('userErrors'):
    ...
    def apply_sales(self, target_date=None):
        ...
        for product in active_sales:
            ...
            print(f"\n{product['product_title']} - {product['color']}")
            print(f"  SKU: {product['sku']}")
            print(f"  Regular: ${product['regular_price']:.2f} → Sale: ${product['sale_price']:.2f} ({product['discount']})")
            
            # Get variant ID if not already present
            variant_id = product.get('variant_id')
            if not variant_id:
                print(f"  Fetching variant ID...")
                variant_id = self.get_variant_id(product['product_id'], product['sku'])
            
            if not variant_id:
                print(f"  ❌ Could not find variant ID")
                error_count += 1
                continue
            
            # Update the price
            if self.update_variant_price(
                variant_id,
                product['sale_price'],
    ...
    def revert_prices(self, date_range):
        ...
        for product in products:
            ...
            print(f"\n{product['product_title']} - {product['color']}")
            print(f"  Reverting to regular price: ${product['regular_price']:.2f}")
            ...
            # Revert the price (no compare_at_price when reverting)
            if self.update_variant_price(variant_id, product['regular_price'], compare_at_price=None):
                success_count += 1
                print(f"  ✅ Price reverted successfully")
            else:
                error_count += 1
                print(f"  ❌ Failed to revert price")
