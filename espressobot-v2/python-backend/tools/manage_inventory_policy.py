...
def update_inventory_policy(variant_id: str, policy: str) -> Dict[str, Any]:
...
def main():
    ...
    parser.add_argument('--policy', '-p', required=True, 
                      ...
                      help='Inventory policy: allow (can oversell) or deny (cannot oversell)')
    ...
    for edge in variants:
        ...
        print(f"  - {variant['sku'] or 'No SKU'} ({variant['title']}): {current_policy} → {policy} [{status}]")
    ...
    # Update each variant
    print(f"\nUpdating inventory policy to {policy}...")
    success_count = 0
    error_count = 0
    ...
    for edge in variants:
        ...
        if variant['inventoryPolicy'] == policy:
            print(f"  ✓ {variant['sku'] or 'No SKU'}: Already set to {policy}")
            success_count += 1
            continue
        
        result = update_inventory_policy(variant['id'], policy)
        ...
        if result.get('data', {}).get('productVariantUpdate', {}).get('userErrors'):
            ...
            print(f"  ✓ {variant['sku'] or 'No SKU'}: Updated to {updated_variant.get('inventoryPolicy', policy)}")
            success_count += 1
