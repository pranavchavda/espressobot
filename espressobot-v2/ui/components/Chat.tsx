...
// import { SeatMap } from "./seat-map";  // Removed for Shopify version
...
export function Chat({ messages, onSendMessage, isLoading }: ChatProps) {
  ...
  // Removed seat map trigger logic for Shopify version
  ...
  // Removed handleSeatSelect for Shopify version
  ...
  return (
    <div className="flex flex-col h-full flex-1 bg-white shadow-sm border border-gray-200 border-t-0 rounded-xl">
      <div className="bg-amber-700 text-white h-12 px-4 flex items-center rounded-t-xl">
        <h2 className="font-semibold text-sm sm:text-base lg:text-lg">
          EspressoBot - Shopify Assistant
        </h2>
      </div>
      {/* Messages */}
      <div className="flex-1 overflow-y-auto min-h-0 md:px-4 pt-4 pb-20">
        {messages.map((msg, idx) => {
          // No special message filtering needed for Shopify version
          return (
            <div
              key={idx}
        ...
        {/* Removed seat map display for Shopify version */}
        {isLoading && (
